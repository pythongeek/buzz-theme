...
<div class="bw-container" style="padding: 80px 5vw;">
    ...
    <!-- Archive Header -->
    <header class="bw-archive-header" style="margin-bottom: 48px; padding-bottom: 32px; border-bottom: 1px solid rgba(255,255,255,0.05);">
        
        <?php if (is_category()) : ?>
            <div style="font-size: 9px; letter-spacing: 0.3em; color: var(--bw-red); text-transform: uppercase; margin-bottom: 8px;">Category</div>
            <h1 class="bw-archive-title" style="font-family: var(--font-display); font-size: clamp(48px, 7vw, 96px); color: var(--bw-white); line-height: 0.9;">
                <?php single_cat_title(); ?>
            </h1>
            <?php if (category_description()) : ?>
                <p class="bw-archive-desc" style="font-size: 14px; color: rgba(255,255,255,0.4); margin-top: 16px; max-width: 600px;">
                    <?php echo category_description(); ?>
                </p>
            <?php endif; ?>
            
        <?php elseif (is_tag()) : ?>
            <div style="font-size: 9px; letter-spacing: 0.3em; color: var(--bw-red); text-transform: uppercase; margin-bottom: 8px;">Tag</div>
            <h1 class="bw-archive-title" style="font-family: var(--font-display); font-size: clamp(48px, 7vw, 96px); color: var(--bw-white); line-height: 0.9;">
                <?php single_tag_title(); ?>
            </h1>
            
        <?php elseif (is_author()) : ?>
            <div style="font-size: 9px; letter-spacing: 0.3em; color: var(--bw-red); text-transform: uppercase; margin-bottom: 8px;">Author</div>
            <h1 class="bw-archive-title" style="font-family: var(--font-display); font-size: clamp(48px, 7vw, 96px); color: var(--bw-white); line-height: 0.9;">
                <?php the_author(); ?>
            </h1>
    ...
    <?php if (have_posts()) : ?>
        <div class="bw-posts-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(350px, 1fr)); gap: 24px;">
            <?php while (have_posts()) : the_post(); ?>
                <article id="post-<?php the_ID(); ?>" <?php post_class('bw-post-card'); ?> style="background: var(--bw-charcoal); padding: 24px; border: 1px solid rgba(255,255,255,0.05); transition: transform 0.2s, border-color 0.2s;">
